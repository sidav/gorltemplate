...
func (aspf *AStarPathfinder) areCoordsValid(x, y int) bool {
	return x >= 0 && y >= 0 && (x < aspf.MapWidth) && (y < aspf.MapHeight)
}
...
func (aspf *AStarPathfinder) FindPath(costFunc func(int, int) int, fromx, fromy, tox, toy int) *Cell {
	...
	for !targetReached {
		...
		//sub-step 2c:
		aspf.analyzeNeighbors(currentCell, &openList, &closedList, costFunc, tox, toy)
		//sub-step 2d:
		total_steps += 1
...
func (aspf *AStarPathfinder) analyzeNeighbors(curCell *Cell, openlist *[]*Cell, closedlist *[]*Cell, costFunc func(int, int) int, targetX, targetY int) {
	...
	for i := -1; i <= 1; i++ {
		for j := -1; j <= 1; j++ {
			...
			x, y := cx+i, cy+j
			if aspf.areCoordsValid(x, y) {
				// if (x != targetX || y != targetY) &&
				if costFunc(x, y) < 0 || getCellWithCoordsFromList(closedlist, x, y) != nil { // Cell is impassable or is in closed list
				...
				if (i * j) != 0 { // the Cell under consideration is lying diagonally
					...
					cost = STRAIGHT_COST * costFunc(x, y)
				}
